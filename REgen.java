...
public class REgen {
     ...
     *    e   trace ast
     *    f   building TDFA
     *    g   building TDFA details
     *    h   history details
     *    i   tstring recognizion
     *    l   building TDFA, summary
     *    k   okui
     *    m   match
     *    n   closure
     *    o   posix compare of Tstrings
     *    p   parse, simplified match
     *    q   configuration dag
     *    r   closure, top
     *    s   sanity check, charts
     *    t   tests
     *    u   benchmark comparison
     * </pre></blockquote><p>
     */
    ...
    private class RandomClass {
        ...
        /** The currently delivered number when deliverin in sequence. */
        double current = 0;
    ...
    private void generateBalanced(){
        ...
        for (int k = 0; k < 3; k++){
            ...
            if (arities == null && k == 2){   // none found, and last attempt
                // impossible
                this.astRoot = null;
    ...
    private void genText(AstNode ast, int l, StringBuilder sb){
        ...
        case A_GRO:
            ...
            int[] arr = null;
            if (ast.groupKind != G_GRO && ast.upperbound >= 0){
                ...
                if ((FL_G & trc) != 0){Trc.out.printf("%sgenText sons budget lenghts of sons %s\n",indent(ast.lev),rangesSumToString(l,ranges,arr));}
            }
            int grlen = 0;
            for (int i = 0; i <= upper; i++){
                ...
                if (rem <= 0) break;
                if (arr == null && this.lenlo < rem && rem < this.lenhi){
                    if ((FL_G & trc) != 0){Trc.out.printf("%sgenText iteration %s last\n",indent(ast.lev),i);}
                    genText(ast.son,rem,sb);
                    break;
                }
            }
            break;
    ...
    private void calcSonLen(int l, AstNode ast){
        ...
        if (ast.upperbound < 0){          // infinite
            if (ast.son.maxStrLen >= 0){
                ...
                if ((FL_G & trc) != 0){Trc.out.printf("%sgenText inf-m:n\n",indent(ast.lev));}
            } else {                      // infinite
                lenlo = ast.son.minStrLen;
                lenhi = l;
                if ((FL_G & trc) != 0){Trc.out.printf("%sgenText inf-inf\n",indent(ast.lev));}
            }
        } else {
            int lower = ast.lowerbound;
            if (lower == 0) lower = 1;     // 0 generates no string, but we come here to have one
            if (ast.son.maxStrLen >= 0){
                lenlo = Math.max(ast.son.minStrLen,l/ast.upperbound);
                lenhi = Math.min(ast.son.maxStrLen,l/lower);
            ...
            if (lenhi < lenlo){
                Trc.out.printf("M:N-m:n no intersection\n");
                System.exit(1);
            }
        }
        if ((FL_G & trc) != 0){Trc.out.printf("%sgenText group {%s,%s}, son %s,%s len %s..%s\n",indent(ast.lev),ast.lowerbound,ast.upperbound,ast.son.minStrLen,ast.son.maxStrLen,lenlo,lenhi);}
        this.lenlo = lenlo;
        this.lenhi = lenhi;
    }
